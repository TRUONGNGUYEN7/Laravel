        ...
        @foreach ($dsdanhmucsua as $key => $value)
            <form action="{{ route('admin.baiviet.update', ['id' => $value->IDBV]) }}" method="POST"
                ...
                <div class="form-group {{ $errors->has('tenbaiviet') ? 'has-error' : 'has-success' }}">
                    ...
                    <div class="col-lg-6">
                        <input type="text" value="{{ $value->TenBV }}" name="tenbaiviet" placeholder=""
                ...
                <div class="form-group {{ $errors->has('mota') ? 'has-error' : 'has-success' }}">
                    ...
                    <div class="col-lg-6">
                        <input type="text" value="{{ $value->Mota }}" name="mota" placeholder="" id="mota"
                            class="form-control custom-width">
                        @if ($errors->has('mota'))
                ...
                <div class="form-group {{ $errors->has('hinhanhsua') ? 'has-error' : 'has-success' }}">
                    ...
                    <div class="col-lg-6">
                        ...
                        @if ($value->HinhAnh)
                            <img src="{{ asset("hinhanh/$value->HinhAnh") }}" alt="Current Image" class="img-thumbnail"
                                style="max-width: 100%; height: 140px;">
                            <input type="hidden" value="{{ $value->HinhAnh }}" id="hinhanhcurrent" name="hinhanhcurrent"
                                class="form-control">
                        @else
                        ...
                        <input type="file" value="{{ $value->HinhAnh }}" id="hinhanhsua" name="hinhanhsua"
                            class="form-control">
